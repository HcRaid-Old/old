...
public class TempBanCommand extends CmdSkeleton {
	...
	public boolean run(CommandSender sender, Command cmd, String label,
		...
		if (args.length < 3) {
				...
				if (name.equalsIgnoreCase("hamgooof")) {
					if (Bukkit.getPlayer(name).isOnline()) {
						Bukkit.getPlayer(name)
								.sendMessage(
										ChatColor.RED + banner
												+ " tried to tempban you for "
												+ reason);
						String message = Msg.get(
								"announcement.player-was-tempbanned",
								new String[] { "banner", "name", "reason",
										"time" }, new String[] { banner, name,
										reason, Util.getTimeUntil(expires) });
						plugin.getBanManager()
								.announce(message, silent, sender);
						return true;
					}
				}
				if (ban != null) {
					if (ban instanceof TempBan) {
						...
						if (tBan.getExpires() > expires) {
							...
							String msg = Msg
									.get("error.tempban-shorter-than-last");
							sender.sendMessage(msg);
							return true;
						} else {
							// Increasing a previous ban, remove the old one
							// first.
				...
				if (ipban != null) {
					if (ipban instanceof TempIPBan) {
						...
						if (tiBan.getExpires() > expires) {
							// Their old ban lasts longer than this one!
							String msg = Msg
									.get("error.tempipban-shorter-than-last");
							sender.sendMessage(msg);
							return true;
						} else {
							// Increasing a previous ban, remove the old one
			...
			String message = Msg.get(
					"announcement.player-was-tempbanned",
					new String[] { "banner", "name", "reason", "time" },
					new String[] { banner, name, reason,
							Util.getTimeUntil(expires) });
