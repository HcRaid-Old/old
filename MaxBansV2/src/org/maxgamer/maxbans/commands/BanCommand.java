...
public class BanCommand extends CmdSkeleton {
	...
	public boolean run(CommandSender sender, Command cmd, String label,
		...
		if (args.length > 0) {
			...
			// The message we will log in history, send to online players, and
			// send across the sync server (if applicable)
			String message = Msg.get("announcement.player-was-banned",
					new String[] { "banner", "name", "reason" }, new String[] {
							banner, name, reason });
