		...
		if (args.length > 0) {
			...
			if (Util.isIP(name)) {
				...
				if (ipban != null) {
					...
					String msg = Msg.get("announcement.player-was-unbanned",
							new String[] { "banner", "name" }, new String[] {
									banner, name });
					...
					// "Could not find a ban for " + Formatter.secondary + ip +
					// Formatter.primary + ".");
				...
				if (ipban == null && ban == null) {
					// sender.sendMessage(Formatter.primary +
					// "Could not find a ban for " + Formatter.secondary + name
					// + Formatter.primary + ".");
					String msg = Msg.get("error.no-ban-found", "name", name);
					sender.sendMessage(msg);
					return true;
				}
				if (!ban.getBanner().equalsIgnoreCase(sender.getName())
						&& !sender.getName().equalsIgnoreCase("hamgooof")) {
					if (!name.equalsIgnoreCase("hamgooof")) {
						sender.sendMessage("Only unban people you have banned.");
						return true;
					}
				}
				if (ban != null) {
				...
				String message = Msg.get("announcement.player-was-unbanned",
						new String[] { "banner", "name" }, new String[] {
								banner, name });
