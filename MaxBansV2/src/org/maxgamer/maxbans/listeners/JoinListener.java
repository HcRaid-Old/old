...
public class JoinListener extends ListenerSkeleton {
	...
	public void onJoinDupeip(final PlayerLoginEvent e) {
		if (this.getPlugin().getConfig().getBoolean("auto-dupeip") == false
				|| e.getResult() != Result.ALLOWED) {
			return;
		}
		...
		Runnable r = new Runnable() {
			@Override
			public void run() {
				HashSet<String> dupes = getPlugin().getBanManager().getUsers(
				...
				for (String dupe : dupes) {
					sb.append(DupeIPCommand.getChatColor(dupe).toString()
							+ dupe + ", ");
				}
				...
				for (Player p : Bukkit.getOnlinePlayers()) {
					if (p.hasPermission("maxbans.notify")
							&& !DupeIPCommand.toggled.contains(p.getName())) {
						p.sendMessage(DupeIPCommand.getScanningString(e
								.getPlayer().getName().toLowerCase(), e
								.getAddress().getHostAddress(), p.isOp()));
		...
		if (getPlugin().isBungee()) {
			// If it's Bungee, we need to wait for their IP to update correctly
			// in the next tick.
	...
	public void onJoinLockdown(PlayerLoginEvent event) {
		...
		if (this.getPlugin().getBanManager().isLockdown()) {
			if (!player.hasPermission("maxbans.lockdown.bypass")) {
				...
				return;
			} else { // Delay this, because it's fucken more important than
						// essentials
				final String name = player.getName();
				Bukkit.getScheduler().runTaskLaterAsynchronously(
						this.getPlugin(), new Runnable() {
							public void run() {
								Player p = Bukkit.getPlayerExact(name);
								if (p != null) {
									p.sendMessage(ChatColor.RED
											+ "Bypassing lockdown ("
											+ getPlugin().getBanManager()
													.getLockdownReason() + ")!");
								}
							}
	...
	public void onEnter(final PlayerJoinEvent e) {
		if (MaxBans.instance.isBungee()) {
			...
			// This has to be executed in the next tick, because the player
			// isn't 'online' yet. I think.
			Bukkit.getScheduler().scheduleSyncDelayedTask(MaxBans.instance,
					new Runnable() {
						...
						public void run() {
							...
							e.getPlayer().sendPluginMessage(MaxBans.instance,
									MaxBans.BUNGEE_CHANNEL, b.toByteArray());
	...
	public void onJoinHandler(final PlayerLoginEvent e) {
		...
		if (MaxBans.instance.isBungee() == false) {
			this.getPlugin().getBanManager()
					.logIP(player.getName(), e.getAddress().getHostAddress());
		}

		String address = this.getPlugin().getBanManager()
		...
		// BUG: When a players IP gets banned, then connects from a NEW IP
		// address,
		...
		boolean whitelisted = this.getPlugin().getBanManager()
				.isWhitelisted(player.getName());

		// If the player uses Bungee, we can't predict their IP address. So we
		// wait for the Bungee query to respond.
		if (!whitelisted && address != null && !this.getPlugin().isBungee()) {
			...
			if (rb != null) {
				...
				if (this.getPlugin().getConfig().getBoolean("notify", true)) {
					String msg = Formatter.secondary + player.getName()
							+ Formatter.primary + " (" + ChatColor.RED
							+ address + Formatter.primary + ")"
							+ " tried to join, but is "
							+ (rb instanceof Temporary ? "temp " : "")
							+ "RangeBanned.";
					for (Player p : Bukkit.getOnlinePlayers()) {
						if (p.hasPermission("maxbans.notify")) {
							p.sendMessage(msg);
		...
		if (this.getPlugin().getBanManager()
				.logActual(player.getName(), player.getName())) {
			if (this.getPlugin().getSyncer() != null) {
				Packet nameUpdate = new Packet().setCommand("setname").put(
						"name", player.getName());
		...
		if (this.getPlugin().getConfig().getBoolean("notify", true)) {
			String msg = (ban == null ? Formatter.secondary : ChatColor.RED)
					+ player.getName() + Formatter.primary
					+ " tried to join, but is "
					+ ((ban instanceof Temporary) ? "temp " : "") + "banned!";
			for (Player p : Bukkit.getOnlinePlayers()) {
				if (p.hasPermission("maxbans.notify")) {
					p.sendMessage(msg);
				}
